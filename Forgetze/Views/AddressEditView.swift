...
struct AddressEditView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                Section("Address Type") {
                    HStack {
                        ...
                        Menu {
                            ...
                            HStack {
                                Text(type)
                                    .foregroundColor(type == "Custom" ? .secondary : .primary)
                ...
                Section("Validation") {
                    HStack {
                        Image(systemName: state.isEmpty ? "circle" : (Address(type: "", street: "", city: "", state: state, zip: "").validateState() ? "checkmark.circle.fill" : "xmark.circle.fill"))
                            .foregroundColor(state.isEmpty ? .secondary : (Address(type: "", street: "", city: "", state: state, zip: "").validateState() ? .green : .red))
                        Text("State Code")
                        Spacer()
                        Text(state.isEmpty ? "Required" : (Address(type: "", street: "", city: "", state: state, zip: "").validateState() ? "Valid" : "Invalid"))
                            .font(.caption)
                            .foregroundColor(state.isEmpty ? .secondary : (Address(type: "", street: "", city: "", state: state, zip: "").validateState() ? .green : .red))
                    }
                    
                    HStack {
                        Image(systemName: zip.isEmpty ? "circle" : (Address(type: "", street: "", city: "", state: "", zip: zip).validateZIP() ? "checkmark.circle.fill" : "xmark.circle.fill"))
                            .foregroundColor(zip.isEmpty ? .secondary : (Address(type: "", street: "", city: "", state: "", zip: zip).validateZIP() ? .green : .red))
                        Text("ZIP Code")
                        Spacer()
                        Text(zip.isEmpty ? "Required" : (Address(type: "", street: "", city: "", state: "", zip: zip).validateZIP() ? "Valid" : "Invalid"))
                            .font(.caption)
                            .foregroundColor(zip.isEmpty ? .secondary : (Address(type: "", street: "", city: "", state: "", zip: zip).validateZIP() ? .green : .red))
                    }
                }
    ...
    private var isValid: Bool {
        return !street.isEmpty && !city.isEmpty && !state.isEmpty && !zip.isEmpty &&
               Address(type: "", street: "", city: "", state: state, zip: zip).validateState() &&
               Address(type: "", street: "", city: "", state: "", zip: zip).validateZIP()
    }
